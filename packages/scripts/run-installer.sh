...
nix flake new --refresh --template "$GITHUB/wyrmling#$TARGET_MACHINE" "/tmp/$TARGET_MACHINE"
...
nix flake new --refresh --template "$GITHUB/wyrmling#$TARGET_MACHINE" /mnt/etc/nixos
sleep 5

echo "Install NixOS into $TARGET_MACHINE..."
nix flake update --flake /mnt/etc/nixos
